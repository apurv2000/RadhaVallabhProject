...
const handleLogin = async () => {
    ...
    try {
      ...
      if (response.ok) {
        await AsyncStorage.setItem('userId', data.user.id); // store user ID
        await AsyncStorage.setItem('userData', JSON.stringify(data.user)); // store full user
        ...
        <TouchableOpacity activeOpacity={0.8} style={{ marginTop: 12 }} onPress={handleLogin}>
          ...
          <View style={styles.bottomLinks}>
            <Text style={styles.signupLink}>
              ...
              <Text style={styles.signupHighlight} onPress={() => router.push('../app')}>
